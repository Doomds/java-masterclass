...
- **Constructor:**
    - Takes an Object and initialises the field value with the parameter that was passed in.
- **Methods:**
...
- It extends `ListItem`.
- **Constructor:**
    - Takes an Object, then calls its parent constructor with the parameter that was passed in.
- **Methods** (all are package-private):
    ...
    - `compareTo()`, takes a ListItem and compares it to the ListItem that called this method. Use value from ListItem for comparison.
        ...
        - If vice versa, then it should return a number less than zero.
        - Return zero if equal.

---

## 3. NodeList (interface)
- **Methods** (all are package-private and abstract):
    - `getRoot()`, `addItem()`, `removeItem()` and `traverse()` (see child class for declaration).
...
- **Fields:**
    - ListItem `root`
- **Constructor:**
    - Takes a ListItem and initialises the field root with the newly passed in parameter.
- **Methods:**
...
- **Methods:**
    ...
    - `traverse()`, takes the root as an argument and does not return anything. It uses recursion to visit all the branches in the tree (Inorder). Print each value on a separate line.

---

## TIPs (Tips for Implementation)
- **TIP 1 (Linked List Rules):**
...
- **TIP 2 (Binary Search Tree Rules):**
...
- **TIP 3 (Inorder Traversal):**
    - Inorder = print the previous node, then the parent node, and then the next node (left -> node -> right).

- **TIP 4 (Formatting Checks):**
    - Be extremely careful with the spelling of the names of the fields, constructors and methods.
    - Be extremely careful about spaces and spelling in the printed output from the traverse() method.
